...
videoFilename = "test4"
inputVideoPath = f"Test Videos/{videoFilename}.mp4"
outputVideoPath = "Output Videos/"

# Poster to inject
poster = cv2.imread("Posters/easports.jpg")
...
out = cv2.VideoWriter(outputVideoPath + f"{videoFilename}_no_Audio.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (fr_w, fr_h))
...
with open(f"Output JSON/{videoFilename}.json", "w") as f:
	json.dump(finalData, f, indent=3)

# Generating final video with audio // Make sure you run the whole video
videoWithAudio = mpe.VideoFileClip(inputVideoPath)
posterVideoWithoutAudio = mpe.VideoFileClip(outputVideoPath + f"{videoFilename}_no_Audio.mp4")
audio_bg = videoWithAudio.audio
posterVideoWithAudio = posterVideoWithoutAudio.set_audio(audio_bg)
posterVideoWithAudio.write_videofile(outputVideoPath + f"{videoFilename}_output.mp4")

if os.path.isfile(outputVideoPath + f"{videoFilename}_no_Audio.mp4"):
	os.remove(outputVideoPath + f"{videoFilename}_no_Audio.mp4")